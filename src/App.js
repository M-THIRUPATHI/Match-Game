...
class App extends Component {
  ...
  tick = () => {
    const {timer, isGameProgress} = this.state
  ...
  renderTabsList = eachTab => {
    const {displayText, tabId} = eachTab
    const onClickTabChanges = () => {
      this.onChangingTabId(tabId)
    }
    return (
      <li className="tabs-list-items" key={tabId}>
        <button
          type="button"
          className="tabs-button"
          onClick={onClickTabChanges}
        >
          {displayText}
        </button>
      </li>
  ...
  onCheckingId = id => {
    ...
    if (id === imagesList[randomIndex].id) {
  ...
  renderThumbnailUrl = eachImageObject => {
    const {thumbnailUrl, id} = eachImageObject
    const onClickThumbnailImage = () => {
      this.onCheckingId(id)
    }
    return (
      <li className="thumbnail-list-items" key={id}>
        <button
          type="button"
    ...
    return (
      <li className="nav-list-items">
        ...
        <div className="timer-container">
          ...
          />
          <p className="timer-view">{timer} sec</p>
        </div>
      </li>
    )
  }
  ...
  render() {
    const {randomIndex, score, isGameProgress} = this.state
    const filterImagesList = this.getfiliterImageList()
    return (
      <div>
        <ul className="nav-container">{this.navElement()}</ul>
          ...
          {isGameProgress ? (
            <>
              ...
              <ul className="tabs-unordered-list">
                {tabsList.map(eachTab => this.renderTabsList(eachTab))}
              </ul>
              <ul className="thumbnails-unordered-list">
                {filterImagesList.map(eachImageObject =>
                  this.renderThumbnailUrl(eachImageObject),
                )}
              </ul>
            </>
          ) : (
